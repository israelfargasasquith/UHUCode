...
[x(:,k+1),laser]=kuta(t(k+1),x,h,0,0,handles_plots,handles_coche,handle_vector,[45 45],MAPA);
 ...
 while (t0+h*k) < tf,
    ...
    %Calcular el vector de percepcion
 ...
 %componentes_x=75;
  ...
  %Para calcular el vector de percercepcion hay que coger las medidas del
  %laser y hacer di = [ri*cosAi ri*senAi], Para hacer hacer el D total hay
  %que meter el subvector de la segunda columna en una funcion sen y en
  %otra funcion cos. Y entonces hay que multiplicar el subvector de las r
  %Todo esto con el vector laser y hay que hacerlo dentro del bucle
  
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % Aquí hay que definir el controlador
  %------------------------------------
  velocidad= 2*sqrt(vPerception(1)^2 +vPerception(2));
  
  volante= 5*atan2(vPerception(2),vPerception(1));
  %------------------------------------
