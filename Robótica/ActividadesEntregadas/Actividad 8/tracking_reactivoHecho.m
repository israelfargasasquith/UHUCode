...
[x(:,k+1),laser]=kuta(t(k+1),x,h,0,0,handles_plots,handles_coche,handle_vector,[45 0],MAPA);
 ...
 rho=2*delta/LH^2;
  ...
  %------------------------------------
  coordinador(k)=coseno;  % coordinador(k)=coseno^3; opcion del compañero
  
  cocienteVolante = 5;
  cocienteVelocidad = 0.2 
  
  volante(k)=(coordinador(k)*volante_tracking(k))+cocienteVolante*((1-coordinador(k))*volante_reactivo(k));
  
  velocidad(k)=(coordinador(k)*velocidad_traking(k))+cocienteVelocidad*((1-coordinador(k))*velocidad_reactiva(k));
...
%metodo de integración ruge-kuta, representación gráfica y simulacion del laser	 
    ...
    %pinta el camino
     ...
     %plot(volante_tracking)
     %plot(velocidad_traking)
